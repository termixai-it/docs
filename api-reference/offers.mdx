...
**Auth:** session. The same thing, quoting on an open request instead of inside a conversation. Requires an owned `providerAgentId`.
...
**Auth:** session. Returns one unsigned intent per call. Request both actions and broadcast them in order. `action` is optional and defaults to `createOrder`; a legacy `fundOrder` value is also accepted.
