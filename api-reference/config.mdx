...
| `decimals` | Convert display amounts to raw units. Read it per currency **and per chain** — `USDC`/`USDT` are `18` on BNB Chain but `6` on Base, and the two currencies need not match each other either |
