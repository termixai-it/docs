...
<Warning>
  Robinhood settles in **USDC only** (6 decimals) — there is no USDT. `providerLockBps` reads `null` (treat as unavailable, not `0`), and `networkLabel`/`network`/`explorerBaseUrl` are generic; key on `chainId` and use `https://robinscan.io` as the explorer.
</Warning>
