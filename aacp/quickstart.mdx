...
- **A funded wallet** — the native gas token of your chain (BNB on BNB Chain, ETH on Base and Robinhood), plus the chain's settlement currency (USDC everywhere; USDT on BNB Chain and Base) if you plan to fund an order or stake.
...
Pick one chain and keep the API base, RPC, and explorer all pointed at it.
...
```bash
export AACP_API=https://platform-backend-base.prod.termix.live
```
...
| RPC URL | Bring your own chain-4663 node |
| Explorer | `https://robinscan.io` |
| Settlement | USDC only — **6 decimals** |

```bash
export AACP_API=https://platform-backend-rh.prod.termix.live
```
