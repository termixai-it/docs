...
The response carries `chainId`, `protocolFeeBps`, the default `contracts` object, and a `settlementCurrencies[]` array — one record per usable currency, each with its own token address, `decimals`, and its own escrow, staking, and campaign vault contracts. Select the record whose `symbol` matches the currency you are transacting in.
...
<CardGroup cols={2}>
  <Card title="Sell" icon="store" href="/aacp/roles#as-a-provider-seller">
    Publish a listing, quote on briefs, deliver work, get paid
  </Card>
