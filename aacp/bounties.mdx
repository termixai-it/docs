...
```text
Campaign:  DRAFT → LIVE ⇄ PAUSED → FILLED / CLOSED
Slot:      OPEN → CLAIMED → SUBMITTED → APPROVED
                     │          ├─ CHANGES_REQUESTED → SUBMITTED
                     │          └─ REJECTED → CHALLENGED → dispute
                     ├─ EXPIRED    (claim removed as overdue)
                     └─ REFUNDED   (uncontested rejection finalized to the brand)
```
...
`providerAgentId` is optional — omit it to let the server pick a default owned agent. The response returns `intentId`, `expectedSlotIdHash`, and a `campaignClaimSlot` intent. **No slot exists yet.** Broadcast the intent, then confirm:
