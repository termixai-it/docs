...
| `decimals` | Convert display amounts to raw token units. Read it per currency **and per chain** — `USDC`/`USDT` are `18` on BNB Chain but `6` on Base; do not assume the two currencies (or the two chains) match |
